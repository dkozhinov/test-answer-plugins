...
import (
	...
	"github.com/dkozhinov/test-answer-plugins/connector-basic/i18n"
...
func (g *Connector) ConnectorSender(ctx *plugin.GinContext, receiverURL string) (redirectURL string) {
	...
	redirectURL = oauth2Config.AuthCodeURL("state", oauth2.AccessTypeOffline)
	return redirectURL
}
...
func (g *Connector) ConnectorReceiver(ctx *plugin.GinContext, receiverURL string) (userInfo plugin.ExternalLoginUserInfo, err error) {
	...
	// Set Authorization header in the token request
	log.Info("Exchanging code for token...")
	token, err := oauth2Config.Exchange(context.Background(), code, oauth2.SetAuthURLParam("client_id", g.Config.ClientID), oauth2.SetAuthURLParam("client_secret", g.Config.ClientSecret))
	if err != nil {
		return userInfo, fmt.Errorf("code exchange failed: %s", err.Error())
	}
	log.Infof("Token received: %s", token.AccessToken)

	// Include Authorization header in the user info request
	...
	request, err := http.NewRequest("GET", g.Config.UserJsonUrl, nil)
	if err != nil {
		return userInfo, fmt.Errorf("failed creating request: %s", err.Error())
	}
	...
	log.Infof("User info successfully retrieved: %+v", userInfo)
	return userInfo, nil
}
